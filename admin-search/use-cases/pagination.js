 ...
 */
 ...
 const writeToFile = async (filename) => {
   try {
     resourceList.forEach((item) => {
       return fs.promises.appendFile(filename, item.public_id + '\n')
     });
   } catch (err) {
 ...
 const getAllResources = async (options,filename) => {
   try {
     do {
       const result = await getResources(options, filename);
     ...
     writeToFile(filename);  //if this was a serverless fn, you could return an array
   } catch (err) {
     console.log(err);
   }
 };
